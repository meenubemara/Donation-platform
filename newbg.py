...
def register_user():
    ...
    aadhaar_path = aadhaar_path_var.get()
    selfie_path = selfie_path_var.get()

    if not aadhaar_path or not selfie_path:
        messagebox.showwarning("Missing Images", "Please upload both Aadhaar and Selfie images.")
        return

    upload_folder = "aadhaar_uploads"
    os.makedirs(upload_folder, exist_ok=True)

    aadhaar_dest = os.path.join(upload_folder, f"{username}_aadhaar.jpg")
    selfie_dest = os.path.join(upload_folder, f"{username}_selfie.jpg")

    shutil.copy(aadhaar_path, aadhaar_dest)
    shutil.copy(selfie_path, selfie_dest)
    ...
    try:
        cursor.execute('''
    INSERT INTO users (name, email, username, password, aadhaar_image, selfie_image, aadhaar_verified)
    VALUES (?, ?, ?, ?, ?, ?, 0)
    ''', (name, email, username, password, aadhaar_dest, selfie_dest))
...
def open_my_donations(user_id):
    ...
    donations_window.configure(bg=BG_COLOR)
    ...
    tk.Label(donations_window, image=logo_photo,font=FONT_NORMAL, bg=BG_COLOR).pack(anchor="ne", padx=10, pady=5)
    donations_window.logo_ref = logo_photo  # Prevent garbage collection

    tk.Label(donations_window, text="My Donations", font=FONT_NORMAL, bg=BG_COLOR).pack(pady=10)
    ...
    if rows:
        ...
        tk.Label(donations_window, text="You haven't made any donations yet.", fg="gray",font=FONT_NORMAL, bg=BG_COLOR).pack(pady=10)
...
def open_chat_window(username):
    ...
    msg_entry = tk.Entry(input_frame,font=FONT_NORMAL, width=30)  # The input box!
    ...
    send_button = tk.Button(input_frame, text="Send", command=send_message,
                            font=FONT_NORMAL, bg=BTN_COLOR, activebackground=BTN_ACTIVE)
...
def open_main_menu(user):
    ...
    logo_image = logo_image.resize((80, 80))
    logo_photo = ImageTk.PhotoImage(logo_image)
    tk.Label(main_menu, image=logo_photo,font=FONT_NORMAL, bg=BG_COLOR).pack(anchor="ne", padx=10, pady=5)
    ...
    tk.Label(main_menu, text=f"Hello, {user[1]}", font=FONT_NORMAL, bg=BG_COLOR).pack(pady=10)
    
    # Check if user is verified
    if user[5] == 1:
        tk.Button(main_menu, text="Donate", width=30, command=lambda: open_donation_screen(user[0]),font=FONT_NORMAL, bg=BTN_COLOR, activebackground=BTN_ACTIVE).pack(pady=10)

    tk.Button(main_menu, text="Register a Need", width=30, command=lambda: open_register_need(user[0]),font=FONT_NORMAL, bg=BTN_COLOR, activebackground=BTN_ACTIVE).pack(pady=10)

    if user[6] == 0:  # Not admin
        tk.Button(main_menu, text="Chat with Admin", width=30,
              command=lambda: open_chat_window(user[1]),
              font=FONT_NORMAL, bg=BTN_COLOR, activebackground=BTN_ACTIVE).pack(pady=10)

    # Show admin panel button if is_admin = 1
    if user[6] == 1:
        tk.Button(main_menu, text="Admin Panel", width=30, command=open_admin_panel,font=FONT_NORMAL, bg=BTN_COLOR, activebackground=BTN_ACTIVE).pack(pady=10)
    tk.Button(main_menu, text="View My Donations", width=30, command=lambda: open_my_donations(user[0]),font=FONT_NORMAL, bg=BTN_COLOR, activebackground=BTN_ACTIVE).pack(pady=10)
    tk.Button(main_menu, text="Chatbot", width=30, command=chatbot,font=FONT_NORMAL, bg=BTN_COLOR, activebackground=BTN_ACTIVE).pack(pady=10)
    tk.Button(main_menu, text="Logout", width=30, command=lambda: (main_menu.destroy(), root.deiconify()),font=FONT_NORMAL, bg=BTN_COLOR, activebackground=BTN_ACTIVE).pack(pady=20)
def open_register_need(user_id):
    need_window = tk.Toplevel()
    need_window.title("Register a Need")
    need_window.geometry("1500x1000")
    need_window.configure(bg=BG_COLOR)
    logo_image = Image.open("C:/Users/meenu/OneDrive/Pictures/Desktop/meenu akka/homepage.jpeg")
    logo_image = logo_image.resize((80, 80))
    logo_photo = ImageTk.PhotoImage(logo_image)
    tk.Label(need_window, image=logo_photo,font=FONT_NORMAL, bg=BG_COLOR).pack(anchor="ne", padx=10, pady=5)
    need_window.logo_ref = logo_photo
    tk.Label(need_window, text="Type of Need",font=FONT_NORMAL, bg=BG_COLOR).pack(pady=5)
    need_type_entry = tk.Entry(need_window)
    need_type_entry.pack()

    tk.Label(need_window, text="Description",font=FONT_NORMAL, bg=BG_COLOR).pack(pady=5)
    description_entry = tk.Entry(need_window)
    description_entry.pack()

    # Auto-detect location
    tk.Label(need_window, text="Location (Auto-filled)",font=FONT_NORMAL, bg=BG_COLOR).pack(pady=5)
    ...
    tk.Label(need_window, text="Full Address",font=FONT_NORMAL, bg=BG_COLOR).pack(pady=5)
    address_var = tk.StringVar()
    tk.Entry(need_window, textvariable=address_var, font=FONT_NORMAL, width=40).pack(pady=5)

    tk.Label(need_window, text="Contact Details",font=FONT_NORMAL, bg=BG_COLOR).pack(pady=5)
    contact_entry = tk.Entry(need_window)
    contact_entry.pack()
    tk.Label(need_window, text="Upload Image",font=FONT_NORMAL, bg=BG_COLOR).pack(pady=5)
    ...
    tk.Button(need_window, text="Choose Image", command=browse_image,font=FONT_NORMAL, bg=BTN_COLOR, activebackground=BTN_ACTIVE).pack()
    ...
    tk.Button(need_window, text="Submit Need", command=submit_need,font=FONT_NORMAL, bg=BTN_COLOR, activebackground=BTN_ACTIVE).pack(pady=20)
def open_donation_screen(user_id):
    ...
    donation_window.configure(bg=BG_COLOR)
    # Add this after you create the window
    logo_image = Image.open("C:/Users/meenu/OneDrive/Pictures/Desktop/meenu akka/homepage.jpeg")
    logo_image = logo_image.resize((80, 80))
    logo_photo = ImageTk.PhotoImage(logo_image)
    ...
    tk.Label(donation_window, text="Select a Need to Donate",font=FONT_NORMAL, bg=BG_COLOR).pack(pady=10)
    ...
    def donate_to_selected():
        ...
        tk.Label(confirm_win, text="Enter a message for the recipient (optional)",font=FONT_NORMAL, bg=BG_COLOR).pack(pady=5)
        donation_msg = tk.Entry(confirm_win, width=40)
        donation_msg.pack(pady=5)

        tk.Button(confirm_win, text="Confirm Donation", command=confirm_donation,font=FONT_NORMAL, bg=BTN_COLOR, activebackground=BTN_ACTIVE).pack(pady=10)

    tk.Button(donation_window, text="Donate to Selected Need", command=donate_to_selected,font=FONT_NORMAL, bg=BTN_COLOR, activebackground=BTN_ACTIVE).pack(pady=10)
...
def show_confirmation_page(need_details, message):
    ...
    tk.Button(confirmation_win, text="Close", command=confirmation_win.destroy,font=FONT_NORMAL, bg=BTN_COLOR, activebackground=BTN_ACTIVE).pack(pady=20)
...
def open_admin_panel():
    ...
    admin_window.geometry("1500x1000")
    ...
    def load_unverified_users():
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT id, name, email, username, aadhaar_verified, aadhaar_image, selfie_image FROM users WHERE is_admin=0")
    ...
    tk.Button(user_tab, text="Verify Selected User", command=verify_selected_user,font=FONT_NORMAL, bg=BTN_COLOR, activebackground=BTN_ACTIVE).pack(pady=10)
    ...
    def load_unverified_needs():
        ...
        for row in cursor.fetchall():
            needs_tree.insert('', 'end', values=row)
        conn.close()
    def preview_user_docs():
        selected = user_tree.selection()
        if not selected:
            messagebox.showwarning("No Selection", "Select a user to preview Aadhaar & Selfie.")
            return
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT aadhaar_image, selfie_image FROM users WHERE id = ?", (user_id,))
        result = cursor.fetchone()
        conn.close()

        if not result or not result[0] or not result[1]:
            messagebox.showinfo("No Images", "No Aadhaar or Selfie found.")
            return

        win = tk.Toplevel()
        win.title("Preview Documents")
        win.geometry("600x300")

        aadhaar_img = Image.open(result[0]).resize((250, 250))
        selfie_img = Image.open(result[1]).resize((250, 250))
        aadhaar_photo = ImageTk.PhotoImage(aadhaar_img)
        selfie_photo = ImageTk.PhotoImage(selfie_img)

        tk.Label(win, text="Aadhaar",font=FONT_NORMAL, bg=BG_COLOR).pack()
        tk.Label(win, image=aadhaar_photo).pack(side="left", padx=20)
        tk.Label(win, text="Selfie",font=FONT_NORMAL, bg=BG_COLOR).pack()
        tk.Label(win, image=selfie_photo).pack(side="right", padx=20)

        win.aadhaar_photo = aadhaar_photo
        win.selfie_photo = selfie_photo
    def mark_user_verified():
        ...
        if selected:
            ...
            conn = get_db_connection()
            cursor = conn.cursor()
            cursor.execute("UPDATE users SET aadhaar_verified = 1 WHERE id = ?", (user_id,))
            conn.commit()
            conn.close()
            messagebox.showinfo("Verified", "User manually marked as Aadhaar Verified.")
    ...
    tk.Button(user_tab, text="Manually Verify User", command=mark_user_verified,
          bg="orange", activebackground="darkorange").pack(pady=5)
    ...
    tk.Button(user_tab, text="Preview Aadhaar & Selfie", command=preview_user_docs,
          bg="lightblue", activebackground="skyblue").pack(pady=5)
...
tabControl.place(relx=0.5, rely=0.5, anchor="center", width=800, height=700)
style = ttk.Style()
style.theme_use("default")
#style = ttk.Style()
#style.theme_use("default")
#style.configure("Treeview", rowheight=30, font=FONT_SMALL)
#style.configure("Treeview.Heading", font=FONT_NORMAL)
corner_img = Image.open("C:/Users/meenu/OneDrive/Pictures/Desktop/meenu akka/homepage.jpeg")
corner_img = corner_img.resize((200, 200))  # Adjust size if needed
corner_photo = ImageTk.PhotoImage(corner_img)

img_label = tk.Label(root, image=corner_photo, bg="#FFA500")
img_label.place(relx=1.0, rely=1.0, anchor="se", x=-20, y=-20)  # Bottom-right corner with padding
root.corner_photo_ref = corner_photo  # Prevent garbage collection
# === Login Tab ===
login_tab = ttk.Frame(tabControl)
tabControl.add(login_tab, text='Login')

login_frame = tk.Frame(login_tab, bg="white")
login_frame.place(relx=0.5, rely=0.5, anchor="center")

tk.Label(login_frame, text="Login", font=FONT_LARGE, bg="white").pack(pady=20)
tk.Label(login_frame, text="Username", font=FONT_NORMAL, bg="white").pack()
login_username = tk.Entry(login_frame, font=FONT_NORMAL, width=30)
login_username.pack(pady=5)

tk.Label(login_frame, text="Password", font=FONT_NORMAL, bg="white").pack()
login_password = tk.Entry(login_frame, show="*", font=FONT_NORMAL, width=30)
login_password.pack(pady=5)

tk.Button(login_frame, text="Login", font=FONT_NORMAL, width=20, height=2,
          bg=BTN_COLOR, fg="white", activebackground=BTN_ACTIVE,
          command=login_user).pack(pady=20)
...
register_frame = tk.Frame(register_tab, bg="white")
register_frame.place(relx=0.5, rely=0.5, anchor="center")

tk.Label(register_frame, text="Register", font=FONT_LARGE, bg="white").pack(pady=20)

tk.Label(register_frame, text="Name", font=FONT_NORMAL, bg="white").pack()
reg_name = tk.Entry(register_frame, font=FONT_NORMAL, width=30)
reg_name.pack(pady=5)

tk.Label(register_frame, text="Email", font=FONT_NORMAL, bg="white").pack()
reg_email = tk.Entry(register_frame, font=FONT_NORMAL, width=30)
reg_email.pack(pady=5)

tk.Label(register_frame, text="Username", font=FONT_NORMAL, bg="white").pack()
reg_username = tk.Entry(register_frame, font=FONT_NORMAL, width=30)
reg_username.pack(pady=5)

tk.Label(register_frame, text="Password", font=FONT_NORMAL, bg="white").pack()
reg_password = tk.Entry(register_frame, show='*', font=FONT_NORMAL, width=30)
reg_password.pack(pady=5)

# Aadhaar & Selfie Upload
aadhaar_path_var = tk.StringVar()
selfie_path_var = tk.StringVar()

def browse_aadhaar():
    file = filedialog.askopenfilename(filetypes=[("Images", "*.jpg *.jpeg *.png")])
    if file:
        aadhaar_path_var.set(file)

def browse_selfie():
    file = filedialog.askopenfilename(filetypes=[("Images", "*.jpg *.jpeg *.png")])
    if file:
        selfie_path_var.set(file)

tk.Label(register_frame, text="Upload Aadhaar Image", font=FONT_NORMAL, bg="white").pack()
tk.Button(register_frame, text="Browse Aadhaar", font=FONT_NORMAL, bg=BTN_COLOR,
          fg="white", activebackground=BTN_ACTIVE, command=browse_aadhaar).pack()
tk.Label(register_frame, textvariable=aadhaar_path_var, wraplength=400, bg="white").pack()

tk.Label(register_frame, text="Upload Selfie", font=FONT_NORMAL, bg="white").pack(pady=5)
tk.Button(register_frame, text="Browse Selfie", font=FONT_NORMAL, bg=BTN_COLOR,
          fg="white", activebackground=BTN_ACTIVE, command=browse_selfie).pack()
tk.Label(register_frame, textvariable=selfie_path_var, wraplength=400, bg="white").pack()

tk.Button(register_frame, text="Register", font=FONT_NORMAL, width=20, height=2,
          bg=BTN_COLOR, fg="white", activebackground=BTN_ACTIVE,
